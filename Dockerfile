...
LABEL org.label-schema.build-date=$BUILD_DATE \
    ...
    org.label-schema.description="Docker For PHP/Laravel Developers - Docker image with PHP 7.3 and NodeJS and Yarn with additional PHP extensions on official PHP Alpine flavour" \
...
RUN php -m \
    && install-php-extensions bcmath calendar exif gmp gd pcntl zip opcache pdo_mysql sockets xdebug redis mongodb \
    && apk add --no-cache \
    git \
    openssh-client \
    wget \
